...
import { Play, RotateCcw, Copy, Settings2, Sparkles, Cpu, Zap, Download } from 'lucide-react';
        ...
        <div className="flex-1 flex flex-col p-8 gap-6">
          <div className={`flex-1 rounded-[2.5rem] p-10 flex flex-col border transition-all duration-700 ${result ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-black border-white/5'}`}>
            <div className="flex justify-between items-center mb-8">
              ...
              {result && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => downloadText(result, 'playground-output.md')}
                    title="Baixar como .md"
                    className="bg-indigo-600/20 hover:bg-indigo-600/40 p-2 rounded-xl text-indigo-400 hover:text-white transition-all"
                  >
                    <Download size={16} />
                  </button>
                </div>
            ...
            <div className="flex-1 overflow-y-auto font-code text-lg leading-relaxed text-slate-300 selection:bg-indigo-500/30">
              {result ? (
                <div className="animate-fade-in whitespace-pre-wrap">{result}</div>
              ) : (
